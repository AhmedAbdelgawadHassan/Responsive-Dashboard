...
class _AllExpnesesItemsListviewState extends State<AllExpnesesItemsListview> {
  ...
  Widget build(BuildContext context) {
    return Row(                // we doesn't use listview.builder because we have to put fixed height to AllExpenesesItem so we use Row insteadof listview.builder
      children: allExpensesItems.asMap().entries.map((e) {      // we use asMap() to get the index of each item
        ...
        if(index==1){   // we use index 1 to give it only padding to second item because we doesn't want to give padding to other items (fitst,third)
